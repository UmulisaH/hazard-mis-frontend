...
import type { AuthResponse, RegisterEmployeeRequest } from '@/lib/auth/types';
import { useAuth } from '@/lib/auth/auth-context';
import type { NormalizedApiError } from '@/lib/api/types';

type RegisterField = keyof RegisterEmployeeRequest;
...
export default function RegisterPage() {
  ...
  const selectedInstitution = useMemo(
    () => institutions.find((institution) => institution.id === institutionId) ?? null,
  ...
  const canSubmit = useMemo(
    ...
    [departmentId, email, fullName, institutionId, jobTitle, password, phone],
  );
  ...
  return (
    <main className="relative min-h-screen overflow-hidden px-4 py-6 sm:px-6 lg:px-10">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(14,165,233,0.16),transparent_26%),radial-gradient(circle_at_bottom_left,rgba(251,191,36,0.22),transparent_30%),linear-gradient(180deg,rgba(255,255,255,0.62),rgba(255,255,255,0.2))]" />
      ...
      <section className="relative mx-auto grid min-h-[calc(100vh-3rem)] max-w-6xl overflow-hidden rounded-[2rem] border border-slate-900/10 bg-white/85 shadow-[0_28px_100px_rgba(15,23,42,0.14)] backdrop-blur lg:grid-cols-[0.9fr_1.1fr]">
        ...
        >
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(251,191,36,0.22),transparent_30%),radial-gradient(circle_at_bottom_left,rgba(14,165,233,0.12),transparent_32%)]" />
          <div className="relative flex h-full flex-col justify-between">
            <div>
              ...
              <h1 className="mt-4 max-w-sm text-4xl font-semibold tracking-tight">
                Create an account and join the workflow.
              </h1>
            </div>
            <div className="mt-10 grid max-w-sm gap-3 text-sm text-slate-200">
              <div className="rounded-3xl border border-white/10 bg-white/8 px-4 py-3 backdrop-blur-sm">
                Register once, use the same workspace everywhere
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="w-full">
            <div className="mb-8 space-y-3">
              ...
              <p className="max-w-xl text-sm leading-6 text-slate-600">
                Add the details required to start reporting hazards.
              </p>
            </div>

            <form className="grid gap-5 sm:grid-cols-2" onSubmit={handleSubmit}>
              <label className="flex flex-col text-sm font-medium text-slate-700 sm:col-span-1">
                Email
                <input
                  type="email"
                  value={email}
              ...
              <label className="flex flex-col text-sm font-medium text-slate-700 sm:col-span-1">
                ...
                <input
                  ...
                  required
                />
                <FieldError message={getFieldMessage(fieldError, 'fullName')} />
              </label>
              ...
              <label className="flex flex-col text-sm font-medium text-slate-700 sm:col-span-1">
                ...
                <input
                  ...
                  required
                />
                <FieldError message={getFieldMessage(fieldError, 'jobTitle')} />
              </label>
              ...
              <label className="flex flex-col text-sm font-medium text-slate-700 sm:col-span-1">
                ...
                <input
                  ...
                  required
                />
                <FieldError message={getFieldMessage(fieldError, 'phone')} />
              </label>
              ...
              <label className="flex flex-col text-sm font-medium text-slate-700 sm:col-span-1">
                ...
                <select
                  ...
                  className="mt-2 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-950 outline-none transition disabled:cursor-not-allowed disabled:bg-slate-100 focus:border-slate-950 focus:bg-white"
                ...
                <FieldError
                  message={getFieldMessage(fieldError, 'institutionId')}
                />
              </label>
              ...
              <label className="flex flex-col text-sm font-medium text-slate-700 sm:col-span-1">
                ...
                <select
                  ...
                  className="mt-2 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-950 outline-none transition disabled:cursor-not-allowed disabled:bg-slate-100 focus:border-slate-950 focus:bg-white"
                  disabled={!selectedInstitution || selectedDepartments.length === 0}
                  required
                >
                  <option value="">
                    {!selectedInstitution
                ...
                <FieldError
                  message={getFieldMessage(fieldError, 'departmentId')}
                />
              </label>
              ...
              <button
                ...
                className="sm:col-span-2 rounded-2xl bg-slate-950 px-4 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-400"
              >
